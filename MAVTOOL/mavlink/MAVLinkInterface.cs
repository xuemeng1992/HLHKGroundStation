...
/**
* 功 能：mavlink设备接口对象
* 类 名：MAVLinkInterface
* 说 明：Mavlinhk库到的方法接口类，里面实现了模式设置上锁，解锁，模式设置，发送指令等基本操作
* Ver 变更日期 负责人 变更内容
* ───────────────────────────────────
...
namespace MAVTOOL.mavlink
{
    public class MAVLinkInterface : MAVLink, IDisposable
    {
        ...
        public void Open(bool getparams, bool skipconnectedcheck = false)
        {
            ...
            frmProgressReporter = new ProgressReporterDialogue //新建一个连接对话框窗口
            {
            ...
            {
                frmProgressReporter.DoWork += FrmProgressReporterDoWorkNOParams;//执行链接函数
            }
            frmProgressReporter.UpdateProgressAndStatus(-1, Strings.MavlinkConnecting);
   
            frmProgressReporter.RunBackgroundOperationAsync();//对话框的异步状态变化界面
...
4. Try a diffrent usb port\n\n" + "No Mavlink Heartbeat Packets where read from this port - Verify Baud Rate and setup\nMission Planner waits for 2 valid heartbeat packets before connecting");
                    ...
                    if (buffer1.Length == 0)
                        buffer1 = getHeartBeat();//尝试读取串口数据得到心跳包
                    ...
                    if (buffer.Length > 5 && buffer1.Length > 5 && buffer.sysid == buffer1.sysid && buffer.compid == buffer1.compid)
                    {
                        ...
                        if (hb.type != (byte)MAVLink.MAV_TYPE.GCS)
                        {
                            ...
                            isconnect = true;//表示已经得到心跳包，并且已经连接上
                            break;
                        }
                    }
                    ...
                    if (buffer.Length > 5 && buffer1.Length > 5 && (buffer.sysid == buffer1.sysid || buffer.compid == buffer1.compid))
                    {
                        ...
                        {
                            SetupMavConnect(buffer, hb);//创建一个mavlink连接
                            break;//连接成功，退出连接状态
                        }
                ...
                if (buffer.Length > 5)
                {
                    if (buffer.msgid == (byte)MAVLINK_MSG_ID.PARAM_VALUE)
                    {
                       ...
                       //log.Info("setParam gotback " + st + " : " + MAVlist[sysid, compid].param[st]);
                        ...
                        {
                            /*
                            dv1.instance.BeginInvoke((Action) delegate
                            {
        ...
        */
        ...
        public void requestDatastream(MAV_DATA_STREAM id, byte hzrate, int sysid = -1, int compid = -1)
        {
           ...
           //lo//g.InfoFormat("Request stream {0} at {1} hz for {2}:{3}",
           ...
           //log.InfoFormat("setWPint {7}:{8} {6} frame {0} cmd {1} p1 {2} x {3} y {4} z {5}", req.frame, req.command, req.param1,
            ...
            while (true)
            {
                ...
                if (buffer.Length > 5)
                {
                    ...
                    {
                        var ans = buffer.ToStructure<mavlink_mission_ack_t>();
                       //log.I//nfo("set wp " + index + " ACK 47 : " + buffer.msgid + " ans " +
                              //   Enum.Parse(typeof(MAV_MISSION_RESULT), ans.type.ToString()));
                        giveComport = false;
                ...
                if ((message.header == 'U' || message.header == 0xfe || message.header == 0xfd) && buffer.Length >= message.payloadlength)
                {
                    ...
                    lock (objlock)
                    {
                        ...
                        if (msgid == (byte)MAVLINK_MSG_ID.ADSB_VEHICLE)
                        {
                            ...
                            //if (dv1.instance.adsbPlanes.ContainsKey(id))
                            //{
                            //    // update existing
                            //    ((adsb.PointLatLngAltHdg)dv1.instance.adsbPlanes[id]).Lat = adsb.lat / 1e7;
                            //    ((adsb.PointLatLngAltHdg)dv1.instance.adsbPlanes[id]).Lng = adsb.lon / 1e7;
                            //    ((adsb.PointLatLngAltHdg)dv1.instance.adsbPlanes[id]).Alt = adsb.altitude / 1000.0;
                            //    ((adsb.PointLatLngAltHdg)dv1.instance.adsbPlanes[id]).Heading = adsb.heading * 0.01f;
                            //    ((adsb.PointLatLngAltHdg)dv1.instance.adsbPlanes[id]).Time = DateTime.Now;
                            //    ((adsb.PointLatLngAltHdg)dv1.instance.adsbPlanes[id]).CallSign = ASCIIEncoding.ASCII.GetString(adsb.callsign);
                            //}
                            //else
                            //{
                            //    // create new plane
                            //    dv1.instance.adsbPlanes[id] =
                        ...
                        if (msgid == (byte)MAVLINK_MSG_ID.COLLISION)
                        {
                            ...
                            //if (dv1.instance.adsbPlanes.ContainsKey(id))
                            //{
                            //    ((adsb.PointLatLngAltHdg)dv1.instance.adsbPlanes[id]).ThreatLevel = threat_level;
                            //}
                        }
                    }
                    ...
                    if (msgid == (byte)MAVLINK_MSG_ID.STATUSTEXT) // status text
                    {
                       ...
                       //log.Info(DateTime.Now + " " + sev + " " + logdata);
                        ...
                        if (printit)
                        {
                            ...
                            //if (dv1.speechEngine != null &&
                            //    dv1.speechEngine.IsReady &&
                            //    Settings.Instance["speechenable"] != null &&
                            //    Settings.Instance["speechenable"].ToString() == "True")
                            //{
                            //    if (speechenabled)
                            //        dv1.speechEngine.SpeakAsync(logdata);
                            //}
                        }
                    }
