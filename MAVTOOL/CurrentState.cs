       ...
       // // [DisplayText("RX Rssi")]
        ...
        public float crit_AOA
        {
            get
            {
                try
                {
                    if (comPort.MAV.param.ContainsKey("AOA_CRIT"))
                    {
                        return
                            (int)
                                (comPort.MAV.param["AOA_CRIT"].Value);
                    }
                    else
                    {
                        return 25;
        ...
        public float GeoFenceDist
        {
            get
            {
                try
                {
                    ...
                    var R = 6371e3;
                    // close loop
                    var list = comPort.MAV.fencepoints.ToList();
                    if (list.Count > 0)
                    {
                ...
                if (DateTime.Now > lastupdate.AddMilliseconds(50) || updatenow) // 20 hz
                {
                    ...
                    if (mavLinkMessage != null)
                    {
                        ...
                        else
                        {
                            ...
                            //if (oldmode != mode && dv1.speechEnable && dv1.comPort.MAV.cs == this &&
                            //    Settings.Instance.GetBoolean("speechmodeenabled"))
                            //{
                            //    dv1.speechEngine.SpeakAsync(Common.speechConversion("" + Settings.Instance["speechmode"]));
                            //}
                        }
                    }
                    ...
                    if (mavLinkMessage != null)
                    {
                        ...
                        //if (oldwp != wpno && dv1.speechEnable && dv1.comPort.MAV.cs == this &&
                        //    Settings.Instance.GetBoolean("speechwaypointenabled"))
                        //{
                        //    dv1.speechEngine.SpeakAsync(Common.speechConversion("" + Settings.Instance["speechwaypoint"]));
                        //}
